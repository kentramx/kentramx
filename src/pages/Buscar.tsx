...
const Buscar = () => {
  ...
  // ✅ Estado para debounce del mapa (evita recargas excesivas)
  ...
  // Este es el puente que faltaba: Mapa -> Coordenadas -> Lista
  const handleMapPositionChange = useCallback((center: { lat: number; lng: number }) => {
    ...
    // Esperar 500ms después de que el usuario deja de mover el mapa
    mapMoveTimerRef.current = setTimeout(() => {
      console.log("🗺️ Mapa estabilizado, actualizando lista en:", center);

      // Actualizar coordenadas para que usePropertySearch recargue la lista
      setSearchCoordinates(center);

      // Opcional: Si quieres que el mapa limpie los filtros de texto para priorizar la ubicación geográfica
      /*
      setFilters(prev => ({
         ...prev,
         estado: '', 
