-- Eliminar función anterior y crear versión con cache completo
...
BEGIN
  ...
  -- Zoom bajo (<13): retornar clusters
  IF p_zoom < 13 THEN
    SELECT jsonb_agg(
      jsonb_build_object(
        'lat', avg_lat,
  ...
  -- Zoom alto (>=13): retornar propiedades individuales
  ELSE
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', p.id,
  ...
  INSERT INTO property_tiles_cache (
    ...
    CASE WHEN p_zoom < 13 THEN v_result->'clusters' ELSE NULL END,
    CASE WHEN p_zoom >= 13 THEN v_result->'properties' ELSE NULL END,
    CASE 
      WHEN p_zoom < 13 THEN jsonb_array_length(COALESCE(v_result->'clusters', '[]'::jsonb))
