-- ========================================
-- 🔧 FIX DEFINITIVO: Cache de tiles sin conflictos 409
-- ========================================
-- Elimina constraint redundante UNIQUE(tile_key) que causaba 409 en prefetch/concurrencia
-- Mantiene solo UNIQUE(tile_key, filters_hash) para caché por tile + filtros
-- Ajusta ON CONFLICT para usar explícitamente el constraint compuesto

-- 1. Eliminar constraint redundante UNIQUE(tile_key) si existe
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'property_tiles_cache_tile_key_key'
  ) THEN
    ALTER TABLE property_tiles_cache 
    DROP CONSTRAINT property_tiles_cache_tile_key_key;
    RAISE NOTICE '✅ Constraint redundante property_tiles_cache_tile_key_key eliminado';
  ELSE
    RAISE NOTICE 'ℹ️ Constraint property_tiles_cache_tile_key_key no existe (ya eliminado)';
  END IF;
END $$;

-- 2. Asegurar que existe el constraint compuesto UNIQUE(tile_key, filters_hash)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'property_tiles_cache_tile_key_filters_hash_key'
  ) THEN
    ALTER TABLE property_tiles_cache 
    ADD CONSTRAINT property_tiles_cache_tile_key_filters_hash_key 
    UNIQUE (tile_key, filters_hash);
    RAISE NOTICE '✅ Constraint compuesto UNIQUE(tile_key, filters_hash) creado';
  ELSE
    RAISE NOTICE 'ℹ️ Constraint compuesto ya existe';
  END IF;
END $$;

-- 3. Crear índice NO único en tile_key para consultas (si no existe)
CREATE INDEX IF NOT EXISTS idx_tiles_cache_key 
ON property_tiles_cache(tile_key);

-- 4. Actualizar get_map_tiles para usar ON CONFLICT explícito en constraint compuesto
...
BEGIN
  ...
  -- 6. Guardar en cache con TTL de 5 minutos usando ON CONFLICT explícito
  ...
  -- 🎯 ON CONFLICT explícito usando el constraint compuesto
